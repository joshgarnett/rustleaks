// This safe Rust adaptation follows Go's strconv/eisel_lemire.go algorithm
// and its Wuffs-generated powers-of-ten table. Go is BSD-3-Clause licensed;
...
pub(crate) fn parse_64(mantissa: u64, exponent_10: i64) -> Option<f64> {
    if mantissa == 0 {
        return Some(0.0);
    }
    if !(-348..=347).contains(&exponent_10) {
        return None;
    }

    let leading_zeros = u64::from(mantissa.leading_zeros());
    let mantissa = mantissa << leading_zeros;
    let mut exponent_2 = (((217_706 * exponent_10) >> 16) + 64 + 1_023) as u64;
    exponent_2 = exponent_2.wrapping_sub(leading_zeros);
    let powers = DETAILED_POWERS_OF_TEN[(exponent_10 + 348) as usize];
    let (mut high, mut low) = multiply_64(mantissa, powers[1]);

    if high & 0x1ff == 0x1ff && low.wrapping_add(mantissa) < mantissa {
        let (wider_high, wider_low) = multiply_64(mantissa, powers[0]);
        let (merged_low, carry) = low.overflowing_add(wider_high);
        let merged_high = high.wrapping_add(u64::from(carry));
        if merged_high & 0x1ff == 0x1ff
            && merged_low.wrapping_add(1) == 0
            && wider_low.wrapping_add(mantissa) < mantissa
        {
            return None;
        }
        high = merged_high;
    ...
    let most_significant_bit = high >> 63;
    let mut result_mantissa = high >> (most_significant_bit + 9);
    exponent_2 = exponent_2.wrapping_sub(1 ^ most_significant_bit);
    if low == 0 && high & 0x1ff == 0 && result_mantissa & 3 == 1 {
        return None;
    }
    ...
    if exponent_2.wrapping_sub(1) >= 0x7ff - 1 {
        return None;
    }
    Some(f64::from_bits(
...
fn multiply_64(left: u64, right: u64) -> (u64, u64) {
    let product = u128::from(left) * u128::from(right);
    ((product >> 64) as u64, product as u64)
}

const DETAILED_POWERS_OF_TEN: &[[u64; 2]] = &[
