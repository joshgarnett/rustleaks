...
pub(crate) fn delta_decode(
    ...
    delta_decode_with_cancel(data, channels, messages, &mut || false)
}

pub(crate) fn delta_decode_with_cancel(
    data: &[u8],
    channels: usize,
    messages: DeltaErrorMessages,
    ...
    out.try_reserve_exact(data.len())
        .map_err(|_| Error::InvalidData("DELTA filter allocation failed"))?;
