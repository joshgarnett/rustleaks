...
impl Program {
    pub fn parse(blob: &[u8]) -> Result<Self> {
        ...
        if bits.read_bit()? != 0 {
            ...
            static_data
                ...
                .map_err(|_| Error::InvalidData("RARVM static data allocation failed"))?;
            for _ in 0..size {
        ...
        instructions
            ...
            .map_err(|_| Error::InvalidData("RARVM instruction allocation failed"))?;
    ...
    pub fn execute(&self, invocation: Invocation<'_>) -> Result<ExecutionResult> {
        self.execute_with_cancel(invocation, &mut || false)
    }
    ...
    pub(crate) fn execute_with_cancel(
        ...
        vm.run_with_cancel(self, cancelled)
    }
}
...
fn parse_instruction(bits: &mut BitReader<'_>, instruction_index: usize) -> Result<Instruction> {
    ...
    operands
        ...
        .map_err(|_| Error::InvalidData("RARVM operand allocation failed"))?;
...
impl Vm {
    fn new(program: &Program, invocation: Invocation<'_>) -> Result<Self> {
        ...
        memory
            .try_reserve_exact(MEMORY_SIZE)
            .map_err(|_| Error::InvalidData("RARVM memory allocation failed"))?;
    ...
    fn run(&mut self, program: &Program) -> Result<ExecutionResult> {
        self.run_with_cancel(program, &mut || false)
    }
...
mod tests {
    ...
    #[test]
    fn parses_more_dense_zero_operand_instructions_than_blob_bytes() {
        let mut bits = BitWriter::new();
        bits.write_bits(0, 1);
        for _ in 0..16 {
            write_opcode(&mut bits, Opcode::Pusha);
        }
        let blob = with_xor(bits.finish());

        let program = Program::parse(&blob).unwrap();

        assert!(program.instructions.len() > blob.len());
        assert!(program.instructions[..16]
            .iter()
            .all(|instruction| instruction.opcode == Opcode::Pusha));
        assert_eq!(program.instructions[16].opcode, Opcode::Ret);
    }
