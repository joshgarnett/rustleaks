...
| Hosted CI | `.github/workflows/` | Current workflow has top-level read-only contents permission and separates ordinary tests from oracle work | Local workflow review only | Every action must be pinned to a reviewed full commit SHA; CodeQL and dependency review are required; untrusted jobs must remain secretless; privileged contexts must not execute untrusted pull request code. Current tag references are not release-ready. |
| Releases | Package metadata and local release checks | Local package normalization, public API snapshot, MSRV check, locked release dry run, and clean external consumers | `just release-dry-run` and `just ci` | No package has been published. A protected environment, trusted publishing, checksums, target-specific SBOMs, attestations, native target evidence, and a verified private reporting route remain release blockers. |
