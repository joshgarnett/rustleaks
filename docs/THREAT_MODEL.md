...
| Releases | Package metadata, `.github/workflows/release-dry-run.yml`, protected `release` environment, and local release checks | Local package normalization, public API snapshot, MSRV check, locked release dry run, clean external consumers, required maintainer approval, and `main`-only environment access | `just release-dry-run`, `just ci`, GitHub environment settings, and the non-publishing release workflow | No package has been published. Trusted publishing, checksums, target-specific SBOMs, attestations, and exact-candidate evidence remain release blockers. |
