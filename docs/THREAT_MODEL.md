...
| Dependencies | `Cargo.lock`, `deny.toml`, `supply-chain/`, `supply-chain-exceptions.toml`, dependency documentation | Locked versions and sources; license, advisory, ban, source, build-script, native-code, unsafe, graph, and vet policy; dated cargo-vet bootstrap reviews | `just security`, exact graph hashes, cargo-deny, fresh RustSec audit, cargo-vet, cargo-geiger, Miri, and owned-unsafe checks | A vet exemption is not an audit. The 63 bootstrap exemptions require review by 2026-11-23 and must decrease as source audits are completed. Dependency unsafe code remains within the documented safe-API boundary. |
| Bazel and local automation | `MODULE.bazel`, `MODULE.bazel.lock`, `cargo-bazel-lock.json`, `BUILD.bazel`, `justfile`, `crates/xtask` | Pinned Bazel and Rust repositories; lockfiles; declared inputs; sandbox-compatible actions; exact tool versions; Bazel-backed public recipes | `just ci`, build-system contract tests, package consumers, target compile matrix, and lock consistency checks | Local caches are not a trust boundary for release provenance. Remote cache policy and untrusted CI execution require Goal 6 configuration. |
| Hosted CI | `.github/workflows/` | Current workflow has top-level read-only contents permission and separates ordinary tests from oracle work | Local workflow review only | Goal 6 must pin every action to a reviewed full commit SHA, add CodeQL and dependency review, keep untrusted jobs secretless, and prohibit privileged execution of untrusted pull request code. Current tag references are not release-ready. |
| Releases | Package metadata and the future Goal 6 release workflow | Local package normalization, public API snapshot, MSRV check, locked release dry run, and clean external consumers | `just package-check`, `just release-dry-run`, and `just ci` | No package has been published. Goal 6 must add a protected environment, trusted publishing, checksums, target-specific SBOMs, attestations, native target evidence, and a verified private reporting route. Release is blocked until then. |
